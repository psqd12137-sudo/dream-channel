...
先在 Godot 中完成一集邪典儿童玩具秀的探索、搭建、战斗、事件、成长、大地图 Boss 终局、结局和表现闭环，再扩展 AI 演员记忆与多人玩法。房屋 PCG 既要保证拓扑和战斗可读性，也要像可被看见制作痕迹的实体节目布景。正式完成度以 Godot README、执行计划和回归测试为准；Web 实验状态不计入产品完成度。

大地图 Boss 第一版：标题 → 后台测试 →「大地图 Boss · 独立试玩」。正式终局沿用本局地图，1/3/5格房间的每个物理格都是一个战斗格；支持跨房移动、整图统一回合制、二楼/地下室楼梯连接、拆锚/击杀、失败结算和续玩。三层终局显示采用共享塔心的蛋糕塔布局，并提供左侧楼层滑轨查看二楼、地面层、地下室和总览；切层不消耗 AP 或推进回合。玩法范围和验证见 [大地图Boss试玩说明](docs/design/大地图Boss试玩说明.md)。
...
- [x] 2026-09-09 正式探索逐步揭示信号锚、侧栏数量提示、固定位置存档与终局继承；旧探索存档按完成记录补齐，已有终局续玩保留原布局。

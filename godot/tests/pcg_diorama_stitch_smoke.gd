...
func _run() -> void:
	...
	_check(generator.prop_count > generator.rooms.size() and generator.prop_layout_is_valid(), "room decoration must fill deterministic toy-show room slot requests")
	...
	for seed_offset in range(2, 26):
		...
		_check(generator.prop_layout_is_valid(), "seed %d must preserve a valid furniture layout" % seed_offset)

...
func run() -> void:
	...
	if "--reference" in OS.get_cmdline_user_args():
		...
		game.tactile_lab.toggle_reference_feature(4)
		await capture("C-material-off-no-dof")
		game.zoom_house_camera(Vector2.ZERO, 0.01)
		await capture("C-material-off-near-no-dof")
		game.tactile_lab.toggle_reference_feature(4)
		await capture("C-material-on-near-no-dof")
		game.tactile_lab.reset_camera()
		await capture("C-material-on-no-dof")

...
实体玩具质感对比新增 **C 参考工坊**：标题 → 后台测试 → 实体玩具质感对比，点击 C 或按 C。绿色切割垫、书本、胶带、零件盘、窗、货架和桌灯围绕原有房间摆放；默认保留原房间材质。A/B/C 保持同一镜头，R 重播共用拼装；工坊外墙随观察方向让开。仅用于测试区，不推广正式地图。`tests/capture_tactile_lab.gd -- --reference` 可录制四个朝向及拼装截图。

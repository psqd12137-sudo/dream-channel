...
1 / 3 / 5 格描述的是同一个大地图房间实例的占地范围；每个完整的格子房实例对应一个副本房间实例。副本的空间布局可以与大地图保持相同的 `footprint_kind` 和 `footprint`，也可以在副本数据中独立调整形状、旋转、墙体、门、地板和家具。房间实例仍以 `room_rules.placed.instance_id` 作为玩法边界，不会因为占用多个格子而拆成多个事件或战斗。

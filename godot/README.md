...
样片入口为“标题 → 后台测试 → 单人三阶段样片”，用于验证一场梦从房间经历到终幕破坏秀的完整因果链。第 4、8、12 个进度节点分别进行一次投入或弃权；候选只来自实际走过的房间，候选不足时会暂缓抽片并允许继续探索。样片存档固定写入 `user://solo_stage_trial_v1.json`，正式续玩存档保持独立。

里程碑 0（参考工坊）、A（阶段与经历）、B（投入／弃权抽片）、C（终幕规则差异）和 D（终幕收束与回顾）的代码接线和自动回归均已具备。D 的人工试玩仍待执行，不能用自动测试代替体验结论。验收边界、运行命令、两局人工试玩清单和 3—5 名外部试玩记录模板见 [单人三阶段样片验收](../docs/design/单人三阶段样片验收.md)。

当前已知限制：抽片面板的房间缩略图仍使用明确的文字回顾占位；终幕规则是低成本差异验证，极端规则层已有回归，但完整战斗平衡和全部素材组合仍需人工检查。电视订购、实体解锁、长期成长和多人选角不计入本轮完成度。

专项回归包括 `tests/solo_three_stage_flow_regression.gd`（投入、弃权、候选不足、阶段恢复、普通房事故与正式存档隔离）、`tests/dream_wake_regression.gd`（终幕结局与玩具静止）以及 `tests/dream_finale_rules_regression.gd`（八种规则组合）。
...
沿用现有票根选择、旋转、确认和翻面揭示流程。房间具有安静的奶油色工作台与青绿色边框，地板纹样降低密度和对比，并绑定房间局部坐标；移动模块时印花不会在表面滑动。涂漆木件、黏土与毛毡分别使用 0.48 / 0.82 / 0.98 粗糙度，地板 0.55、图案墙面 0.88。探索使用柔和主光、天空反射与 Forward+ 接触暗部；战斗恢复既有中性照明。

确认后零件从画面外分批坠落，夸张翻转、压扁回弹并卡合，再自动沿合法路径进入新房间；不可达时保留原有手动进入。角色不得穿过未访问房间抄近路，抵达后只到 `room_ready`，事件仍由玩家确认。已提交但被中断的拼装按原存档恢复，不在读档时自动补播入场。

专项测试：`tests/toyhouse_regression.gd`、`tests/formal_cartoon_assembly_regression.gd`（加 `-- --capture` 可记录真实扩建过程）。正式扩建与已认可的 `tests/toy_parts_demo.gd` 共用 `scripts/toy_room_assembly.gd` 的动作和节奏，卡通落件、卡合停顿与角色跳入均已接入正式流程。未知房间仍在进入后揭示家具；美术测试会预先展示家具。详细记录见 [实施计划及验收记录](../docs/superpowers/plans/2026-09-09-memphis-toyhouse.md)。

### 玩具屋完整流程样片（2026-09-09）

标题 → 后台测试 → **玩具屋完整流程样片**可直接观看固定演出：玄关旁生成一个大房间模块，底座、地板、外框与门洞从画面外按组翻转落下，压扁摇摆后卡合，最后一颗小圆帽迟到回弹；莉莉随后带预备压缩和夸张弧线跳入新模板，落脚回弹后才揭示房间。样片使用真实的扩建、合法路径、墙体缩入底座和房间揭示逻辑，但不写入正式续玩存档。

样片内按 `R` 重播，拖拽旋转镜头、滚轮缩放；右侧阶段提示依次显示“准备对位 / 零件落下 / 卡合停顿 / 角色跳入 / 房间就绪”。回归脚本为 `tests/toyhouse_sequence_regression.gd`，可加 `-- --capture` 输出落件与就绪画面到 `output/toyhouse-sequence-drop.png`、`output/toyhouse-sequence-ready.png`。

### 实体玩具质感对比（测试区）

标题 → 后台测试 → **实体玩具质感对比**。同一正式长廊与玄关布局，可点击 A「当前画面」/ B「质感样板」，也可按 A / B 切换。四项独立开关为外围桌面、材质层次、接触阴影、轻景深；拖拽旋转、滚轮缩放，R 重播共用的卡通拼装与角色跳入，右侧可复位镜头。

B 增加桌面、三块积木、贴纸纸张与收纳托盘；材质按涂漆木件、塑料、黏土、毛毡和纸张区分，部分盒形部件使用真实圆角几何；原模型与材质资源不修改。柔光和较窄范围的接触阴影随测试配置切换。轻景深仍使用现有带状模糊，近景会软化远侧墙件，因此 B 默认关闭，仅保留试验开关。A 恢复原先画面配置，包括进入测试前的景深偏好。

这套表现只在测试区启用，等待样板审核。布局、取景与正式存档不随 A/B 改变；退出恢复环境、材质和景深。回归：`tests/toyhouse_tactile_lab_regression.gd`；截图与 30 秒 A/B 性能采样：`tests/capture_tactile_lab.gd -- --benchmark`，输出在 `output/tactile-lab/`。

## 动态效果基线

- 摆放房间：共用已认可测试的 `1.5` 倍放慢节奏，拼装约 `3.03s`。每个零件按当前镜头计算高度，初始完整位于视口上方至少 `48px`；底座、地板、墙件按组落下，保留 `55–100°` 翻转、下落拉伸、落地压扁摇摆与迟到圆帽。房间根节点保持最终位置，落稳后保留 `0.10s` 卡合停顿再衔接合法入场。候选房间仍随旋转按钮即时对齐门向。
- 进入房间：莉莉先用 `0.09s` 预备压缩，再以 `0.40s` 夸张弧线跳入新房间（最高约 `0.42` 格），落脚以 Back 缓出回弹；样片阶段会自动跟随角色，正式探索仍保持原有路径和 `room_ready` 边界。
...
地编现在先选择 `1 / 3 / 5 格`规格，再选择该规格内的中文房名，最后选择编辑大地图或副本。`living` 为 1 格客厅，`parlor` 为 3 格会客室。具体形状是独立的编辑控件，不再作为房间列表的规格入口。

副本是否存在默认取正式内容的战斗标记；映射清单可通过 `has_dungeon` 显式覆盖。无副本的房间禁用副本入口。正式战斗优先读取映射的副本 JSON，使用其中的占地、旋转、家具、墙门及 fixtures；无文件时保留原布景回退。空间表现不生成敌人、事件或逻辑碰撞，战斗阻挡与高差仍由 arena 数据控制。导出自定义布局 ID 时，需要同步在映射清单中指定该 ID 才会作为该房间默认布局加载。

完整一局的后续验收依据见 `../docs/design/完整一局验收清单.md`。新增 `tests/complete_run_flow_regression.gd` 覆盖门连接移动、逐房推进、中途续玩、奖励生效和胜败结局；战斗结果在此测试中模拟，不替代人工通关和平衡验证。

1 / 3 / 5 格描述的是同一个大地图房间实例的占地范围；有副本的格子房实例对应一个副本房间实例，事件/静室允许没有副本。副本的空间布局可以与大地图保持相同的 `footprint_kind` 和 `footprint`，也可以在副本数据中独立调整形状、旋转、墙体、门、地板和家具。房间实例仍以 `room_rules.placed.instance_id` 作为玩法边界，不会因为占用多个格子而拆成多个事件或战斗。

后台测试 → 房间资产地编的顶部“编辑层”可以在两种空间目标之间切换：

- `大地图格子房`：继续读写 `data/editor/overrides/<room_id>.json`，保持原有正式房间覆盖和导出格式。
- `副本房间`：以当前正式房间作为来源，读写 `data/editor/dungeon_layouts/<layout_id>.json`；默认布局 ID 由 `data/editor/dungeon_layout_manifest.json` 映射为 `<room_id>_dungeon_01`。

副本文件不存在时，地编工具会将对应的大地图覆盖或程序生成布局作为初始空间快照；在副本层点击导出只会创建/更新副本布局，不会改写大地图覆盖。副本布局文件第一阶段只保存空间表现字段：占地、旋转、资产、墙体和 fixtures，不保存 `enemies`、战斗棋盘、卡牌、事件或奖励；这些内容继续由独立的房间玩法与战斗数据层决定。这样可以先分别调通“地图上怎么盖”和“进入副本后长什么样”，再由同一房间映射接入运行时。
...
C 参考工坊第二轮：默认使用一体底座造型，可通过“造型：新版/旧版”比较结构。第四个开关恢复原版移轴景深（5.5 / 0.18），默认开启，关闭可检查造型。退出后恢复原显示设置。

C 参考工坊材质迭代：新增“资产分材质”开关，按资产路径和部件名称将局内资产分为哑光塑料、涂装木材、布料、金属、纸张、陶瓷、玻璃和黏土；保留原 albedo 贴图与孟菲斯颜色，叠加低对比微表面变化和不同粗糙度/金属度。桌椅、书架、床品、书籍、灯具、窗和角色因此有不同的光泽响应。只在 C 测试档启用，A/B 和正式地图不受影响；关闭开关可同镜头看原材质。

实体玩具质感对比新增 **C 参考工坊**：标题 → 后台测试 → 实体玩具质感对比，点击 C 或按 C。绿色切割垫、书本、胶带、零件盘、窗、货架和桌灯围绕原有房间摆放；默认保留原房间材质。C 提供“玩具底座与拼图接口”“扣合收尾”“工坊氛围光”三个独立开关。A/B/C 保持同一镜头，R 重播共用拼装；工坊外墙随观察方向让开，每次只处理一个最近墙面方向，避免旋转时整套墙像换了布局。**设计口径：C 参考工坊已提升为正式现实玩具层的视觉基准；实现口径：当前仍仅在测试区运行，尚未推广到正式地图。** `tests/capture_tactile_lab.gd -- --reference --benchmark` 可录制四个朝向、近远景、拼装截图并测量三个档位的性能。正式拼装的扣合扩展默认关闭。

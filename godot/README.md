...
标题 → 后台测试 → **玩具屋完整流程样片**可直接观看固定演出：玄关旁生成一个大房间模块，房间根节点先从画面上方缓慢落下，底座、外框、门洞、楼梯和道具按轻微错拍依次出现，并在卡合后停顿一下；莉莉随后带预备压缩和夸张弧线跳入新模板，落脚回弹后才揭示房间。样片使用真实的扩建、合法路径、墙体缩入底座和房间揭示逻辑，但不写入正式续玩存档。
...
- 摆放房间：完整房间绕共同中心以 `12°` 轻微倾角落位，悬高约 `0.92` 格、初始 `94%` 缩放；根节点在 `0.38s` 内落下并用 `0.12s` 卡合收稳，底座、外框、门洞、楼梯和道具按 `0.06–0.14s` 错拍、总计约 `0.44s` 依次从上方落入，落稳后保留 `0.10s` 卡合停顿再衔接合法入场。候选房间仍随旋转按钮即时对齐门向。

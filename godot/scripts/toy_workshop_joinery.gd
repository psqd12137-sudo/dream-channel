...
func build(lab) -> void:
	...
	for node in lab.host.house_root.find_children("Base_*", "MeshInstance3D", true, false):
		...
		base_records.append({"node": node, "original": node.mesh, "toy": _base_mesh(node.mesh.size, joins), "cell": cell})
...
func set_enabled(value: bool) -> void:
	...
	for record in base_records:
		record.node.mesh = record.toy if enabled else record.original
	for record in hidden_records:
		record.node.visible = false if enabled else record.visible
	for node in added:
		node.visible = enabled
	lab.reference_root.position.y = mat_offset if enabled else 0.0
...
func _base_mesh(size: Vector3, joins: Dictionary) -> ArrayMesh:

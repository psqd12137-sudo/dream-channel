...
func _draw_combat_card(rect: Rect2, card_id: String, card: Dictionary, cost: int, selected: bool, tilt_deg: float = 0.0, alpha_override: float = -1.0) -> void:
	...
	_draw_centered(str(cost), Rect2(cost_center - Vector2.ONE * scaled.call(15.0), Vector2.ONE * scaled.call(30.0)), cost_font_size, Color(INK if accent.get_luminance() > 0.55 else TEXT, fade.a))
	_label(_shorten(str(card.get("name", card_id)), 7), rect.position + Vector2(scaled.call(42.0), scaled.call(24.0)), title_font_size, Color(INK, fade.a))
	_label(_card_kind_label(kind), rect.position + Vector2(scaled.call(42.0), scaled.call(42.0)), kind_font_size, Color(accent.darkened(0.28), fade.a))
	...
	_draw_wrapped_lines(_shorten(str(card.get("text", "")), 36), Vector2(rect.position.x + scaled.call(12.0), art_rect.end.y + scaled.call(12.0)), rect.size.x - scaled.call(24.0), body_font_size, Color("514940", fade.a), 3)
	if unaffordable:

...
func _init() -> void:
	...
	_check(not content.get("run_rules", {}).has("dice_faces"), "production run rules must not expose the archived 0/1/2 speed dice")
	...
	if not living.is_empty():
		...
		combat.setup(arena, living.get("enemy", {}), content["cards"], ["focus", "jab", "brace", "tonic"], 7, content.get("run_rules", {}), content.get("active_relics", []))
		...
		_check(combat.energy == combat.base_energy + combat.energy_bonus and combat.energy_rolls.is_empty(), "Godot combat must use fixed AP plus relic bonus without speed dice")
